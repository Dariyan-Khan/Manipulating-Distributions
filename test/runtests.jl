...
import ManipulatingDistributions: gammaleft, gammaright, legendreseries,
                                  ...
                                  legendre_same_length, legendre_conv
...
@testset "legendre convolution" begin
    ...
    @testset "general intervals" begin
        # Lengths of intervals are an integer multiple of each other
        @testset "Integer multiple intervals" begin
            n = 1000
            f = x -> x^3 + x^2
            g = x-> x^4
            ...
            function h_poly2(x::Real)
                if x in 11..17
                    return 33175439/56 - (2111464*x)/7 + (501383*x^2)/10 - (11431*x^3)/5 - (256*x^4)/3 + x^7/105 + x^8/280
                elseif x in 17..29
                    return 97426272/7 - (44582016*x)/7 + (5573376*x^2)/5 - (444624*x^3)/5 + 2748*x^4
                elseif x in 29..35
                    return -(145501015625/8) + 2528031250*x - (237994375*x^2)/2 + 1863125*x^3 + (8500*x^4)/3 - x^7/105 - x^8/280
                else
                    return 0
                end
            end

            θ = range(11, 35, length=2n+1)[1:end-1]

            @test h.(θ) ≈ h_poly2.(θ)

        end

        @testset "Ratio is between 1 and 2" begin
            # n = 1000
            # f = x -> x^3 - 2x
            # g = x-> x^4 - 7
            ...
            #     if x in 5..8
            #         return 11705/24 - (4698*x)/7 + 261*x^2 - (70*x^3)/3 - (7*x^4)/4 - x^6/15 + x^8/280
            #     elseif x in 8..10
            #         return 341523/8 - (1386732*x)/35 + 13734*x^2 - (10812*x^3)/5 + (525*x^4)/4
            #     elseif x in 10..13
            #         return -(15682797/8) + (29489508*x)/35 - 116657*x^2 + (64964*x^3)/15 + 133*x^4 + x^6/15 - x^8/280
            #     else
            #         return 0
            #     end
            # end
            ...
            function h_poly3(x::Real)
                if x in 3..5
                    return 15/4 - 2*x - x^2/2 + x^4/12
                elseif x in 5..6
                    return -60 + (56*x)/3
                elseif x in 6..8
                    return -((64*x)/3) + 8*x^2 - x^4/12
                else
                    return 0
                end
            end

            θ = range(5, 6, length=2n+1)[1:end-1]
            @test h.(θ) ≈ h_poly3.(θ)
